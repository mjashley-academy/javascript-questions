...
When we call `game.next("Yes").value`, the previous `yield` is replaced with the value of the parameters passed to the `next()` function, `"Yes"` in this case. The value of the variable `answer` is now equal to `"Yes"`. The condition of the if-statement returns `false`, and `JavaScript loves you back â¤ï¸` gets logged.
...
With a _for-of_ loop, we can iterate over **iterables**. An array is an iterable. When we iterate over the array, the variable "item" is equal to the element it's currently iterating over, `"â˜•"` `"ðŸ’»"` `"ðŸ·"` `"ðŸ«"` get logged.
...
With the `call` method, we can change the object to which the `this` keyword refers. In **functions**, the `this` keyword refers to the _the object that the function belongs to_. We declared the `setTimeout` function on the _global object_, so within the `setTimeout` function, the `this` keyword refers to the _global object_. On the global object, there is a variable called _status_ with the value of `"ðŸ˜Ž"`. When logging `this.status`, `"ðŸ˜Ž"` gets logged.
...
In JavaScript, primitive data types (everything that's not an object) interact by _value_. In this case, we set the value of the `favoriteFood` property on the `info` object equal to the value of the first element in the `food` array, the string with the pizza emoji in this case (`'ðŸ•'`). A string is a primitive data type, and interact by value (see my [blogpost](https://www.theavocoder.com/complete-javascript/2018/12/21/by-value-vs-by-reference) if you're interested in learning more)
...
getFruit([['ðŸŠ', 'ðŸŒ'], ['ðŸ']])
getFruit()
getFruit([['ðŸ'], ['ðŸŠ', 'ðŸŒ']])
```

- A: `null`, `undefined`, ðŸŒ
- B: `[]`, `null`, ðŸŒ
- C: `[]`, `[]`, ðŸŒ
- D: `undefined`, `undefined`, ðŸŒ
...
First, we're trying to log the second item in the `['ðŸ']` subarray of `[['ðŸŠ', 'ðŸŒ'], ['ðŸ']]`. This subarray only contains one item, which means there is no item on index `1`, and returns `undefined`.
...
Lastly, we're trying to log the second item in the `['ðŸŠ', 'ðŸŒ']` subarray of `['ðŸ'], ['ðŸŠ', 'ðŸŒ']`. The item on index `1` within this subarray is `ðŸŒ`, which gets logged.
...
Then, we invoke the `splice` method on the fruit array. The splice method does modify the original array, which means that the fruit array now consists of `['ðŸŠ', 'ðŸŽ']`.
At last, we invoke the `unshift` method on the `fruit` array, which modifies the original array by adding the provided value, â€˜ðŸ‡â€™ in this case,  as the first element in the array.  The fruit array now consists of `['ðŸ‡', 'ðŸŠ', 'ðŸŽ']`.
...
Logging `animals[dog]`, or actually `animals["object Object"]` since converting the `dog` object to a string results `"object Object"`, returns the `{ emoji: "ðŸˆ", name: "Sara" }`.
...
The `Promise.all` method runs the passed promises in parallel. If one promise fails, the `Promise.all` method _rejects_ with the value of the rejected promise. In this case, `promise3` rejected with the value `"Third"`. Weâ€™re catching the rejected value in the chained `catch` method on the `runPromises` invocation to catch any errors  within the `runPromises` function. Only `"Third"` gets logged, since `promise3` rejected with this value.
